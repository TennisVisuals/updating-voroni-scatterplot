...
function scatterPlot() {
    ...
    var events = {
       'element': { 'click': null, 'mouseover': null, 'mouseout': null },
    ...
    function chart(selection) {
        selection.each(function () {
            ...
            if (dom_parent.attr('id')) {
               options.width = Math.min($('#' + dom_parent.attr('id')).width(), 1000) - options.margins.left - options.margins.right;
            ...
            // append children g
            var chartWrapper = root.append("g").attr("class", "chartWrapper").attr('id', 'chartWrapper' + options.id)
                              .attr("transform", "translate(" + options.margins.left + "," + options.margins.top + ")");
            var circleGroup =  chartWrapper.append("g").attr("class", "circleWrapper"); 
            var voronoiGroup = chartWrapper.append("g").attr("class", "voronoiWrapper");

            if (options.display.legend && options.legend.dom_element) {
               var legendTitle = options.legend.dom_element.append('div').attr('class', 'legendTitle')
               var legendText = options.legend.dom_element.append('div').attr('class', 'legendText')
            ...
            d3.select(options.display.reset).on("click", resetClick);

            update = function(resize) {
                // boolean resize used to disable transitions during resize operation
 
                if (options.axes.x.scale == 'log') {
                    var xScale = d3.scale.log().range([0, options.width])
                } else {
                    var xScale = d3.scale.linear().range([0, options.width])
                }
                if (options.axes.x.domain) { 
                    xScale.domain(options.axes.x.domain); 
                } else {
                    xScale.domain(d3.extent(_data_set, function(d) { return +d[options.data.x]; }))
                }
                if (options.axes.x.nice) { xScale.nice(); }
                ...
                if (options.axes.y.scale == 'log') {
                    var yScale = d3.scale.log().range([options.height, 0])
                } else {
                    var yScale = d3.scale.linear().range([options.height, 0])
                }
                if (options.axes.y.domain) { 
                    yScale.domain(options.axes.y.domain); 
                } else {
                    yScale.domain(d3.extent(_data_set, function(d) { return +d[options.data.y]; }))
                }
                if (options.axes.y.nice) { yScale.nice(); }
                ...
                var rScale = d3.scale.sqrt()
                    ...
                    .domain(d3.extent(_data_set, function(d) { return +d[options.data.r_scale]; }));
                ...
                update_circleGroups.exit()
                  .attr("r", function() { if (animate) return 10; })
                  .transition().duration(animate ? transition_time : 0)
                  .attr("r", 0)
                  .remove()

                update_circleGroups
                  .attr("class", function(d,i) { return "data_rows" + options.id + " A" + i; })
                  .style("opacity", options.opacityCircles)
                  .style("fill", function(d) {return colors[d[options.data.group]]; })
                  .transition().duration(transition_time)
                  .attr("cx", function(d) {return xScale(d[options.data.x]); })
                  .attr("cy", function(d) {return yScale(d[options.data.y]); })
                  .attr("r", function(d) { 
                     if (highlight && 
                         d[options.data.identifier].toLowerCase().indexOf(highlight.toLowerCase()) >= 0 && 
                ...
                var voronoi = d3.geom.voronoi()
                     .x(function(d) { return xScale(d[options.data.x]); })
                     .y(function(d) { return yScale(d[options.data.y]); })
                     .clipExtent([[0, 0], [options.width, options.height]]);

                voronoiGroup.selectAll(".voroni" + options.id).remove();

                voronoiGroup.selectAll(".voroni" + options.id)
                   .data(voronoi(_data_set))
                  .enter()
                   .append("path")
                   .attr("id", function(d, i) { return "voroni" + options.id + i; })
                   .attr('gen', function(d, i) { return genPath(d, i); } )

                function genPath(d, i) {
                   if (!d) return;
                   d3.select('#voroni' + options.id + i)
                     .datum(d)
                     .style("stroke", function(f) { return options.display.voroni ? "#2074A0" : undefined })
                     .attr("class", function(d) { return "voroni" + options.id + " A" + i; }) 
                     .style("fill", "none")
                     ...
                     .on('click', function(d, i) { if (events.element.click) { events.element.click(d.point, i, this); } })
               ...
               /****** update labels *******/
               chartWrapper.selectAll('.label' + options.id).remove();
               ...
               if (!mobileScreen && options.display.legend && options.legend.dom_element) {
                  ...
                  var legend = legendWrapper.selectAll('.legendSquare')  	
                    .data(Object.keys(colors).map(function(key){return colors[key]}))                              

                  legend.enter()
                    .append('g')   
                    .attr('class', 'legendSquare') 
                    .attr("transform", function(d,i) { return "translate(" + 0 + "," + (i * options.legend.rowHeight) + ")"; })
                  ...
                  legendWrapper.selectAll('.bubbleLegend').remove();

                  bubbleSizeLegend = legendWrapper.append("g").attr('class', 'bubbleLegend')
                    .attr("transform", "translate(" + (options.legend.width/2 - 30) + "," + (Object.keys(colors).length*options.legend.rowHeight + 20) +")");

                  if (options.display.bubble_legend) { bubbleLegend(bubbleSizeLegend, rScale); }
               }
               else {
                  // mobile device
            ...
            function selectLegend(opacity) {
               return function(d, i) {
                  ...
                  chartWrapper.selectAll(".data_rows" + options.id)
                     .filter(function(d) { return d[options.data.group] != chosen; })
                     .transition()
    ...
    chart.data = function(value) {
        ...
        data_set.sort(function(a,b) { return b[options.data.r_scale] > a[options.data.r_scale]; });
        _data_set = JSON.parse(JSON.stringify(data_set));
        highlight = undefined;
        return chart;
    };
    ...
    function showTooltip (d, i, element) {
       ...
       $(element).popover({
          ...
          html : true,
          content: function() { 
             return "<span style='font-size: 11px; text-align: center;'>" + d[options.data.identifier] + "</span>"; }
       });
